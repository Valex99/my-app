import { fallbackLng, locales } from '@/i18n/locales/settings'
...
export async function middleware(request: NextRequest) {
  ...
  const currentUserCookie = cookieStore.get('currentUserBeestro')
  ...
  if (isRestrictedPage && !isLoggedIn) {
    ...
    if (pathnameIsMissingLocale) {
      ...
      const newUrl = `/${fallbackLng}${pathname}${searchParams ? `?${searchParams}` : ''}`
