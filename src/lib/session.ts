...
export async function getSession(): Promise<Session | null> {
  const cookieStore = await cookies()
  const cookie = cookieStore.get('currentUserBeestro')
  ...
  if (cookie?.value) {
    try {
      ...
      console.error('Error parsing or decrypting currentUser cookie:', error)
    }
  }
  return null
}
...
export async function deleteCurrentUser() {
  ...
  cookieStore.delete('currentUserBeestro')
}
